export const resumeData = {
    name: "Zahoor Khan",
    role: "MERN Stack Developer",
    email: "zahoorshinwari5371@gmail.com",
    phone: "+923359194083",
    location: "Pakistan",
    education: [
        {
            school: "Islamia College University Peshawar",
            degree: "BS Computer Science",
            period: "2020 - 2024",
            location: "Peshawar",
            cgpa: "3.78"
        }
    ],
    experience: [
        {
            company: "Instly Technologies",
            role: "Software Developer",
            period: "Jan 2025 - Present",
            location: "Bangkok",
            description: "Building production-scale applications using the MERN stack. Focused on creating fast, reliable user interfaces and integrating intelligent AI features into core business workflows."
        },
        {
            company: "Codestar",
            role: "MERN Stack Developer",
            period: "Aug 2024 - Oct 2024",
            location: "Peshawar",
            description: "Developed and launched modern web applications. Streamlined development processes and ensured high performance across all delivered projects."
        }
    ],
    skills: {
        languages: ["JavaScript", "Python", "C++"],
        frameworks: ["React", "Next.js", "Node.js", "Express", "Tailwind CSS"],
        tools: ["Git", "GitHub", "AWS EC2", "Docker", "API Design"],
        databases: ["MongoDB", "PostgreSQL", "MySQL", "Firebase"]
    },
    projects: [
        {
            title: "Stellar AI",
            role: "AI Health Assistant",
            tech: ["React", "Node.js", "MySQL", "Tailwind", "Eleven Labs"],
            description: "A smart AI platform that helps users navigate health insurance. Uses real-time intelligence to simplify complex coverage analysis and patient queries.",
            link: "https://askstellarai.com"
        },
        {
            title: "AI Nurse",
            role: "Virtual Clinic",
            tech: ["React", "Node.js", "MySQL", "Eleven Labs", "Tailwind"],
            description: "Making healthcare more accessible. AI-powered symtom analysis and guidance to help patients find the right care faster.",
            link: "https://askainurse.com"
        },
        {
            title: "Insured Nomads",
            tech: ["React", "Node.js", "MySQL", "Tailwind", "Flywire"],
            description: "A seamless travel insurance portal with global payment support, designed for modern travelers needing quick and easy protection."
        },
        {
            title: "BSI",
            tech: ["React", "Node.js", "MySQL", "Tailwind"],
            description: "Digital platform for insurance brokers to manage policies and communicate with clients more efficiently."
        }
    ],
    certifications: [
        "Full Stack Web Development - Udemy",
        "IoT System Development - NV&TTC"
    ],
    socials: {
        github: "https://github.com/zahoorkhan266",
        linkedin: "https://www.linkedin.com/in/zakhori-shinwari-373b14391?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app"
    }
};
